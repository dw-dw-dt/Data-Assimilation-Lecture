...
t  <- 0:(Nt-1) * dt  # temporal coordinate
dx <- 10.0           # interval for spatial difference
Nx <- 400            # number of spatial grids
x  <- 0:(Nx-1) * dx  # spatial coordinate

x1  <- 1:Nx           # set of spatial indices
xl1 <- c(2:Nx,1)      # leftward shifted of x1
xr1 <- c(Nx,1:(Nx-1)) # rightward shifted of x1

Np   <- 1000          # number of ensemble members in EnKF

xini <- 50:150 # set of indices for initial condition
...
exact <- function( u ) { # analytical solution
...
for ( k in 2:Nt ) {
  ...
  xtp[Nx+1, ] <- xtf[Nx+1, ] # + rnorm( Np, mean=0, sd=0.1 ) # no system noise
